...
class Healer:
    ...
    def heal_troop(self,troop):
        if(self.alive==False):
            return
        if(self.King.position[0]==troop.position[0] and self.King.position[1]==troop.position[1]):
            self.heal(self.King,1)
        else:
            self.heal(troop,0)

    def heal(self, target, isKing):
        if(self.alive == False):
            return
            ...
            for col in range(j, j+3):
                ...
                for troop in troops:
                    if(troop.position[0] == row and troop.position[1] == col):
                        troop.health=min(troop.max_health,troop.health+self.heal_strength)
    ...
    def move(self, troop):
        ...
        if(self.isInAttackradius(troop.position)):
...
def move_healers():
    for hl in healers:
        if(hl.alive == False):
            continue
        closest_troop = search_for_closest_troop(hl.position,hl.King)
        hl.move(closest_troop)
...
def search_for_closest_troop(pos,King):
    closest_troop=None
    closest_dist = 10000
    troops=barbarians+archers+stealtharchers+balloons+dragons+[King]
    for tr in troops:
        pos_tr=tr.position
        a=pos_tr[0]
        b=pos_tr[1]
        man_dist=abs(pos[0]-a)+abs(pos[1]-b)
        if(man_dist<closest_dist and tr.health<tr.max_health):
