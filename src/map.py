...
def printMap(V):
    ...
    for hl in healers:
        a= 2*hl.position[0]
        b = 2*hl.position[1]
        map_matrix[a][b] = archColor(hl) + Fore.WHITE + 'HE'
        map_matrix[a+1][b] = archColor(hl) + Fore.WHITE + '  '
        map_matrix[a][b+1] = archColor(hl) + Fore.WHITE + 'AL'
        map_matrix[a+1][b+1] = archColor(hl) + Fore.WHITE + 'ER'

    for arch in stealtharchers:
        if time.time() - arch.startTime <= 10:
            a= 2*arch.position[0]
            b = 2*arch.position[1]
            map_matrix[a][b] = sarchColor(arch) + Back.WHITE + 's.'
            map_matrix[a+1][b] = sarchColor(arch) + Back.WHITE + 'CH'
            map_matrix[a][b+1] = sarchColor(arch) + Back.WHITE + 'AR'
            map_matrix[a+1][b+1] = sarchColor(arch) + Back.WHITE + 'ER'
        else :
            a= 2*arch.position[0]
            b = 2*arch.position[1]
            map_matrix[a][b] = archColor(arch) + Fore.BLACK + 's.'
            map_matrix[a+1][b] = archColor(arch) + Fore.BLACK + 'CH'
